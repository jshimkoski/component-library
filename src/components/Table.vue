<template>
  <div class="w-full overflow-x-auto bg-white dark:bg-base-950 border border-base-200 dark:border-base-800 rounded-base">
    <!-- Table controls -->
    <div v-if="$slots.controls || selectable" class="p-4 flex items-center justify-between">
      ...
      <div v-if="selectedItems.length > 0">
        <span class="text-sm text-base-600 dark:text-base-400">{{ selectedItems.length }} selected</span>
      </div>
    </div>

    <table class="w-full border-collapse text-left">
      <thead class="bg-base-50 dark:bg-base-900 border-b border-base-200 dark:border-base-800">
        <tr>
          <!-- Selection column -->
          ...
          <th v-if="expandable" class="px-4 py-3 w-[40px]">
          ...
          >
            <div class="flex items-center gap-1">
              {{ field.label }}
              <span v-if="field.sortable" class="text-base-400">
                <svg v-if="sortBy === field.key && sortDir === 'asc'" class="w-4 h-4" viewBox="0 0 16 16" fill="currentColor">
                  <path d="M4.5 10.5L8 7l3.5 3.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" fill="none" />
                </svg>
                <svg v-else-if="sortBy === field.key && sortDir === 'desc'" class="w-4 h-4" viewBox="0 0 16 16" fill="currentColor">
                  <path d="M4.5 6L8 9.5 11.5 6" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" fill="none" />
                </svg>
                <svg v-else class="w-4 h-4 text-base-300 dark:text-base-700" viewBox="0 0 16 16" fill="currentColor">
                  <path d="M4.5 6L8 9.5 11.5 6M4.5 10.5L8 7l3.5 3.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" fill="none" />
                </svg>
              </span>
            </div>
          </th>
      ...
      <tbody class="divide-y divide-base-200 dark:divide-base-800">
        <template v-for="(item, index) in sortedItems" :key="getItemId(item)">
          <!-- Data row -->
          <tr
            :class="[
          ...
          >
            ...
            <td v-if="expandable" class="px-4 py-3">
              ...
              >
                <svg 
                  ...
                  :class="{ 'rotate-180': expandedRows.includes(getItemId(item)) }"
                  viewBox="0 0 16 16" 
                  fill="none"
                >
                  <path d="M4.5 6L8 9.5 11.5 6" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                </svg>
              </button>
            </td>
            ...
            >
              ...
              >
                ...
                <span v-if="field.type === 'number'">{{ formatNumber(getItemValue(item, field.key)) }}</span>
                <span v-else-if="field.type === 'date'">{{ formatDate(getItemValue(item, field.key)) }}</span>
                <span v-else-if="field.type === 'boolean'">
                  <svg v-if="getItemValue(item, field.key)" class="w-4 h-4 text-success-500" viewBox="0 0 16 16" fill="currentColor">
                    <path d="M12.207 4.793a1 1 0 010 1.414l-5 5a1 1 0 01-1.414 0l-2-2a1 1 0 011.414-1.414L6.5 9.086l4.293-4.293a1 1 0 011.414 0z" />
                  </svg>
                  <svg v-else class="w-4 h-4 text-base-400" viewBox="0 0 16 16" fill="currentColor">
            ...
            <!-- Actions cell -->
            <td v-if="$slots['row-actions']" class="px-4 py-3 whitespace-nowrap">
          ...
          >
            <td :colspan="colspan" class="px-4 py-3 border-t border-base-100 dark:border-base-800">
              <slot name="expanded-content" :item="item" :index="index">
                <pre class="text-sm whitespace-pre-wrap">{{ JSON.stringify(item, null, 2) }}</pre>
              </slot>
            </td>
          </tr>
        </template>
        ...
        <tr v-if="!loadingItems && items.length === 0">
          <td :colspan="colspan" class="px-4 py-6 text-center text-base-500 dark:text-base-400">
            <slot name="empty-state">
              <div class="flex flex-col items-center justify-center py-6">
                <svg class="w-10 h-10 mb-2 text-base-300 dark:text-base-700" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <!-- Pagination -->
    <div v-if="$slots.pagination" class="border-t border-base-200 dark:border-base-800 p-4">
