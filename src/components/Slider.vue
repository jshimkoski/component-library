<template>
  <div class="slider-container">
    <div class="mb-2 flex items-center justify-between">
      <label v-if="label" :for="id" class="block text-sm font-medium text-base-700 dark:text-base-300">
        {{ label }}
      </label>
      <div v-if="showValue" class="text-xs font-medium text-base-500 dark:text-base-400">
    ...
    <div class="relative flex items-center">
      ...
      <div 
        ...
        :class="{ 'opacity-50 cursor-not-allowed': disabled, 'ring-2 ring-primary-500': isFocused }"
    ...
    <!-- Optional tick marks -->
    <div v-if="showTicks && steps > 0" class="relative mt-2 px-2">
      <div 
        ...
        :class="{ 'bg-primary-500 dark:bg-primary-400': modelValue >= tick.value }"
      ></div>
    </div>
  </div>
</template>
...
const startDrag = (event: MouseEvent | TouchEvent) => {
  ...
  const cleanup = () => {
    window.removeEventListener('mousemove', moveHandler as EventListener, { passive: false } as EventListenerOptions);
    window.removeEventListener('touchmove', moveHandler as EventListener, { passive: false } as EventListenerOptions);
...
const incrementValue = () => {
  ...
  emit('focus'); // Indicate focus with keyboard
};
